...
filterMenu.addEventListener('touchstart', function(event) {
    const target = event.target;
    if (target.tagName.toLowerCase() === 'li') {
        target.classList.add('active'); // Add a styling class for "active" state
    }
});

filterMenu.addEventListener('touchend', function(event) {
    const target = event.target;
    if (target.tagName.toLowerCase() === 'li') {
        target.classList.remove('active'); // Remove the styling class
    }
});
</script>
@endpush